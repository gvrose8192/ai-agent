...
def get_prompt(file_handle):
    ...
    if choice == '1':
        ...
        # Step 1: Ask for the question first
        question = input("Enter your question about the file: ")
        
        loaded_content = ""
        
        # Step 2: Loop to ask for files until the user hits enter with no file specified
        while True:
            filename = input("\nEnter the full path to your text file, or hit enter to finish loading files: ").strip()
            
            if not filename:  # Check if the user hit enter with no file specified
                break
            ...
            try:
                with open(filename, 'r') as f:
                    ...
                    print(f"[LOG] Loaded {len(content)} characters from '{filename}'", file=file_handle, flush=True)
            except FileNotFoundError:
                print(f"Error: File not found at '{filename}'. Skipping to next...")
                # Continue loop to allow user to enter another valid filename
                continue 
            except Exception as e:
                print(f"Error reading file: {e}")
                continue

        # Step 3: Construct the prompt based on whether files were loaded
        if loaded_content.strip():
            combined_prompt = f"What is the answer to the following question: '{question}' based on this text? {loaded_content}"
        else:
            # If no files are specified, just run the question prompt (without file content)
            combined_prompt = f"What is the answer to the following question: '{question}'"
